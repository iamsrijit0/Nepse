...
The CSV file contains one row per signal (not per symbol). Each signal includes:

| Column | Description |
|--------|-------------|
| **Symbol** | Stock ticker symbol |
| **Date** | When the signal was generated |
| **Close_at_Signal** | Price when signal occurred (buy price) |
| **Latest_Close** | Current price (sell price) |
| **P/L_%** | Profit/Loss if you bought at signal and held |
| **Status** | STRICT_PASS, STANDARD_ONLY, or NO_SIGNALS |
| **Standard_Signals** | Total signals in standard mode for this symbol |
| **Strict_Signals** | Total signals in strict mode for this symbol |
...
profitable = df[df['P/L_%'] > 0].sort_values('P/L_%', ascending=False)
print("Top 10 profitable signals:")
print(profitable[['Symbol', 'Date', 'Close_at_Signal', 'Latest_Close', 'P/L_%']].head(10))

# Find symbols with most signals
signals_by_symbol = df[df['Status'] != 'NO_SIGNALS'].groupby('Symbol').size()
print("\nSymbols with most signals:")
print(signals_by_symbol.sort_values(ascending=False).head(10))
